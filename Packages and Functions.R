...
#direct <- paste0("E:/Jacks Package")

wd_setup <- function(basedir = "E:/Jacks Package",save.levels = 2){
  #browser()
  dirs <- list.dirs(basedir)
  
  wds <- strsplit(dirs,split = "/")
  baselength <- length(strsplit(basedir,split = "/"))
  ...
  dirs <- as.list(dirs)
  names(dirs) <- foldernames
  return(dirs)
}


## Vector Probability for LOLP Longterm planning
vector.probability.v2 <- function(x, sample.size = 10000,key.cols = c("UNIT.ID","Month","Hour"),Cap.col = "Interval.Capacity") {
  ...
  # - EFOR = The estimate possibility the vector will not be available
